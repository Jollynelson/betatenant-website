...
const ROLES = [
  { value: "user",     label: "I'm a Tenant",   subtitle: "Find apartments, book rentals, and move in faster.", icon: User },
  { value: "landlord", label: "I'm a Landlord",  subtitle: "List properties, earn rent, and grow your income.", icon: Home },
...
function SignupContent() {
  ...
  // selection. Always start fresh at role step.
  useEffect(() => {
  ...
  // Generate slug preview from agency name
  ...
  return (
    <div className="min-h-[calc(100dvh-72px)] flex items-center justify-center px-5 py-10">
      <AnimatePresence mode="wait">
        ...
        {/* ── Step 1: Role ──────────────────────────────────────── */}
        {step === "role" && (
          <motion.div key="role" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="w-full max-w-md">
            <div className="text-center mb-8">
              <Link href="/"><Image src="/images/bt_logo_big.svg" alt="Beta Tenant" width={140} height={36} className="h-9 w-auto mx-auto mb-6" /></Link>
            ...
            <div className="space-y-3">
              {ROLES.map((r) => (
                <button key={r.value} onClick={() => selectRole(r.value)}
                  className={cn("w-full flex items-center gap-4 p-4 rounded-2xl border-2 text-left transition-all hover:border-bt-primary/50",
                    role === r.value ? "border-bt-primary bg-bt-primary/5" : "border-neutral-200 bg-white")}>
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <Field label="First name" error={errors.firstName}>
                  <input type="text" placeholder="First name" value={form.firstName}
                    onChange={(e) => setForm({ ...form, firstName: e.target.value })} className={inputCls(!!errors.firstName)} />
                </Field>
                <Field label="Last name" error={errors.lastName}>
                  <input type="text" placeholder="Last name" value={form.lastName}
                    onChange={(e) => setForm({ ...form, lastName: e.target.value })} className={inputCls(!!errors.lastName)} />
                </Field>
              </div>

              {/* Agency / Company name — only for agents and landlords */}
              {isAgentOrLandlord && (
                <Field label={`${role === "landlord" ? "Company / Agency" : "Agency"} Name (optional)`}>
                  <div className="relative">
                    <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400" />
                    <input
                      type="text"
                      placeholder={role === "landlord" ? "e.g. Sunshine Properties" : "e.g. Hauz of Floxy"}
                      value={agencyName}
                      onChange={(e) => setAgencyName(e.target.value)}
                      className={cn(inputCls(false), "pl-11 pr-11")}
                    />
                  ...
                  {agencyName.trim().length >= 3 && slugPreview && (
                    <p className={cn("text-xs mt-1.5 flex items-center gap-1",
                      slugAvailable === true ? "text-emerald-600" : slugAvailable === false ? "text-amber-600" : "text-neutral-400")}>
                      {slugAvailable === false
                        ? `⚠ "betatenant.com/in/${slugPreview}" is taken — you can change it later`
                        : slugAvailable === true
                          ? `✓ Your link will be: betatenant.com/in/${slugPreview}`
                          : `Your link: betatenant.com/in/${slugPreview}`}
                    </p>
                  )}
                  <p className="text-xs text-neutral-400 mt-1">This becomes your shareable portfolio link. You can change it anytime.</p>
                </Field>
              )}
              ...
              <Field label="Email address" error={errors.email}>
                <div className="relative">
                  ...
                  <input type="email" placeholder="Enter your email address" value={form.email}
                    onChange={(e) => setForm({ ...form, email: e.target.value })} className={cn(inputCls(!!errors.email), "pl-11")} />
                </div>
              </Field>
              ...
              <Field label="Password" error={errors.password}>
                <div className="relative">
                  ...
                  <input type={showPassword ? "text" : "password"} placeholder="Enter your preferred password" value={form.password}
                    onChange={(e) => setForm({ ...form, password: e.target.value })} className={cn(inputCls(!!errors.password), "pl-11 pr-11")} />
              ...
              <label className="flex items-start gap-2.5 text-sm cursor-pointer">
                ...
                <span className="text-neutral-500">
                  ...
                  and <Link href="/privacy-policies" className="text-bt-primary underline">Privacy Policy</Link>
                </span>
              </label>

              <button type="submit" disabled={!form.agree || loading}
              ...
              {/* Google — keeps role context via localStorage for the callback */}
              <button type="button"
                onClick={() => {
                  // Store role so Google callback knows what role to assign
                  localStorage.setItem("BT_GOOGLE_ROLE", role);
                  if (isAgentOrLandlord && agencyName.trim()) {
                    localStorage.setItem("BT_PENDING_AGENCY", agencyName.trim());
                  }
          ...
          <motion.div key="verify" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="w-full max-w-md">
            <div className="text-center mb-8">
              ...
              <p className="text-neutral-500 mt-1">
                We sent a verification code to <span className="font-semibold text-neutral-700">{form.email}</span>
              </p>
            </div>
            <form onSubmit={handleVerify} className="space-y-4">
              <Field label="Enter verification code">
                <div className="relative">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400" />
                  <input type="text" placeholder="000-000" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ""))}
                    maxLength={6} className={cn(inputCls(false), "pl-11 text-center text-xl tracking-[0.3em] font-bold")} />
                </div>
              </Field>
              <button type="submit" disabled={loading || otp.length < 4}
                className="w-full py-3.5 rounded-full bg-bt-primary text-white font-semibold disabled:opacity-50">
            ...
            <p className="text-center text-neutral-500 text-sm mt-5">
              Wrong email?{" "}
              <button onClick={() => setStep("form")} className="underline text-bt-secondary font-semibold">Change email</button>
            </p>
          </motion.div>
        )}
...
function inputCls(hasError: boolean) {
  return cn(
    ...
    hasError ? "border-red-400 focus:ring-red-100" : "border-neutral-200 focus:ring-bt-primary/20 focus:border-bt-primary"
  );
}
